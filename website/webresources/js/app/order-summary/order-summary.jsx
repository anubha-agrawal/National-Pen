...
class OrderSummary extends Component {
    ...
    placeOrder() {
        ...
        let cartMeta = this.state.cartMeta; 
        let paymentState = 'Paid'; 
        let orderState = 'Confirmed'; 
        let orderTransitionState = 'Confirmed'; 
        let customFields = {    
            deviceIPAddress : '127.0.0.1' 
        }; 
        console.log('Calling Abc.order.createOrder'); 
        console.log(cartMeta); 
        
        
        
        Abc.order.createOrder(cartMeta, customFields, paymentState, orderState, orderTransitionState).then(res => {    
            this.changeGenericHandler('isLoader', false);
            console.log('Placed order ', JSON.stringify(res));    
            console.log('orderMeta', { id:res.id, version:res.version }); 
            let orderNo = res.orderNumber;
            window.location.href = 'order-confirmation?orderno='+ orderNo;
